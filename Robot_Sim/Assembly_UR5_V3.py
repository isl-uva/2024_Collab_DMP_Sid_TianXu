...
def init_setup_robot():
    ...
    table_vis_id = p.createVisualShape(shapeType=p.GEOM_BOX,halfExtents=[0.5,0.8,offset[2]],visualFramePosition=[0, 0, 0], rgbaColor=[0.6, 0.4, 0, 1])
...
if __name__=="__main__":
    ...
    while True:
        ...
        Last_position = list(x_ee)

        # Start_position = Last_position
        # Last_position[2] = Last_position[2] - 0.2
        # Last_position[0] = Last_position[0] + 0.4
        ...
        start = [0.4,-0.2,0.2]
        start_ori = [1.57, 0.0, -3.14]
        end = [0.4,0.2,0.4]
        end_ori = [1.57, 0.0, -4.71]
        for i in range(11):               
            pos_ee_ = i/10*np.array(end) + (1-i/10)*np.array(start)
            pos_ee = pos_ee_.tolist()
            ori_ee_euler_ = i/10*np.array(end_ori) + (1-i/10)*np.array(start_ori)
            ori_ee_euler = ori_ee_euler_.tolist()
            # ori_ee_euler = [1.57, 0.0, -3.14]
            ori_ee_euler.reverse()
            ori_ee = p.getQuaternionFromEuler(ori_ee_euler)
            joint_angles = p.calculateInverseKinematics(robot_id,ee_index, pos_ee, ori_ee,maxNumIterations=100)
        ...
        start = [0.4,0.3,0.2]
        start_ori = [1.57, 0.0, -4.71]
        end = [0.4,0.3,0.4]
        end_ori = [-1.57, 0.0, -3.14]
        for i in range(11):               
            pos_ee_ = i/10*np.array(end) + (1-i/10)*np.array(start)
            pos_ee = pos_ee_.tolist()
            ori_ee_euler_ = i/10*np.array(end_ori) + (1-i/10)*np.array(start_ori)
