...
def create_3d_grid_world(objs, block_size, obs, block_mass, offset): 
    ...
    for i,ob in enumerate(obs):      
        ...
        color = [0,0,0]
        p.createMultiBody(baseMass=block_mass, baseCollisionShapeIndex=box_id, basePosition=position, baseOrientation=[0, 0, 0], baseVisualShapeIndex=box_id)
    ...
    while True:
        ...
        for j,kt in enumerate(Traj_data):
            ...
            if j == 1 or j==4 or j==0:
                ...
                for i in range(11):               
                    pos_ee_ = i/10*np.array(block_pos) + (1-i/10)*np.array(Last_position)
