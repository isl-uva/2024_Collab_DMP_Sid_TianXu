...
from dmp.dmp_ import DMPs_cartesian_
...
def gen_3d_plot():
    ...
    x_max = np.array(desired_x[0]).max()+0.3
    y_max = np.array(desired_y[0]).max()+0.3
    z_max = np.array(desired_z[0]).max()+0.3
    x_min = np.array(desired_x[0]).min()-0.3
    y_min = np.array(desired_y[0]).min()-0.3
    z_min = np.array(desired_z[0]).min()-0.3



    # Plot the first part
    ax1 = fig.add_subplot(131, projection='3d')
    ax1.plot(x1_part1_m, y1_part1_m, z1_part1_m, color='r', label='Measured 1')
    ax1.plot(x1_part1_d, y1_part1_d, z1_part1_d, color='g', label='Desired 1')
    ax1.plot(x2_part1_m, y2_part1_m, z2_part1_m, color='orange', label='Measured 2')
    ax1.plot(x2_part1_d, y2_part1_d, z2_part1_d, color='blue', label='Desired 2')
    ax1.set_title('t = T/3')
    ax1.set_xlabel('X')
    ax1.set_ylabel('Y')
    ax1.set_zlabel('Z')
    ...
    ax2.plot(x1_part2_m, y1_part2_m, z1_part2_m, color='r', label='Measured 1')
    ax2.plot(x1_part2_d, y1_part2_d, z1_part2_d, color='g', label='Desired 1')
    ax2.plot(x2_part2_m, y2_part2_m, z2_part2_m, color='orange', label='Measured 2')
    ax2.plot(x2_part2_d, y2_part2_d, z2_part2_d, color='blue', label='Desired 2')
    ...
    ax3.plot(x1_part3_m, y1_part3_m, z1_part3_m, color='r', label='Measured 1')
    ax3.plot(x1_part3_d, y1_part3_d, z1_part3_d, color='g', label='Desired 1')
    ax3.plot(x2_part3_m, y2_part3_m, z2_part3_m, color='orange', label='Measured 2')
    ax3.plot(x2_part3_d, y2_part3_d, z2_part3_d, color='blue', label='Desired 2')
    ax3.set_title('t = T')
    ax3.set_xlabel('X')
    ax3.set_ylabel('Y')
    ax3.set_zlabel('Z')
...
def gen_xyz_plot():
    ...
    t_des_1 = np.linspace(0.0, 0.01+0.01*(len(x1_positions_m)), len(x1_positions_m))
    t_des_2 = np.linspace(0.0, 0.01+0.01*(len(x2_positions_m)), len(x2_positions_m))

    # # Plot x vs time
    # axs[0].plot(t_des_1, x1_positions_m, label='X Position (Arm 1)', color='r', marker='v', markersize='5')
    # axs[0].plot(t_des_2, x2_positions_m, label='X Position (Arm 2)', color='g', marker='+', markersize='5')
    ...
    # # Plot y vs time
    # axs[1].plot(t_des_1, y1_positions_m, label='Y Position (Arm 1)', color='r', marker='v', markersize='5')
    # axs[1].plot(t_des_2, y2_positions_m, label='Y Position (Arm 2)', color='g', marker='+', markersize='5')
    ...
    # # Plot z vs time
    # axs[2].plot(t_des_1, z1_positions_m, label='Z Position (Arm 1)', color='r', marker='v', markersize='5')
    # axs[2].plot(t_des_2, z2_positions_m, label='Z Position (Arm 2)', color='g', marker='+', markersize='5')
    ...
    # Plot x vs time
    axs[0].plot(t_des_1, x1_positions_m, label='X Position (Arm 1)', color='r', 
                marker='v', markersize=5, markerfacecolor='orange', markeredgewidth=1.2)
    axs[0].plot(t_des_2, x2_positions_m, label='X Position (Arm 2)', color='g', 
                marker='o', markersize=5, markerfacecolor='yellow', markeredgewidth=1.2)
    axs[0].set_title('End-Effector Position vs Time')
    ...
    # Plot y vs time
    axs[1].plot(t_des_1, y1_positions_m, label='Y Position (Arm 1)', color='r', 
                marker='v', markersize=5, markerfacecolor='orange', markeredgewidth=1.2)
    axs[1].plot(t_des_2, y2_positions_m, label='Y Position (Arm 2)', color='g', 
                marker='o', markersize=5, markerfacecolor='yellow', markeredgewidth=1.2)
    # axs[1].set_title('Y Position vs Time')
    ...
    # Plot z vs time
    axs[2].plot(t_des_1, z1_positions_m, label='Z Position (Arm 1)', color='r', 
                marker='v', markersize=5, markerfacecolor='orange', markeredgewidth=1)
    axs[2].plot(t_des_2, z2_positions_m, label='Z Position (Arm 2)', color='g', 
                marker='o', markersize=5, markerfacecolor='yellow', markeredgewidth=1)
    # axs[2].set_title('Z Position vs Time')
...
if __name__=="__main__":
    ...
    robot2.initialize(base_pos=[0.8, 0.2, 0.0],    base_ori=[0,0,-1,0])
    ...
    # box_id1 = pb.loadURDF("Robot_Sim/urdf/object/box/box.urdf", (0.3, 0.0, 0.025))
    # box_id2 = pb.loadURDF("Robot_Sim/urdf/object/box/box.urdf", (0.3, 0.0, 0.075))
    # box_id3 = pb.loadURDF("Robot_Sim/urdf/object/box/box.urdf", (0.45, 0.45, 0.025))

    # pb.changeVisualShape(box_id1, -1,-1, rgbaColor = COLORS['red'])
    # pb.changeVisualShape(box_id2, -1,-1, rgbaColor = COLORS['green'])
    # pb.changeVisualShape(box_id3, -1,-1, rgbaColor = COLORS['pink'])

    start1_0 = robot1._getLinkState(robot1.end_effector_index)[0]
    start2_0 = robot2._getLinkState(robot2.end_effector_index)[0]
    # Stacking Trajectory
    Traj1 = [[0.30, 0.50, 0.05], 
             [0.30, 0.0, 0.05],
             start1_0]
    Traj2 = [[0.30, -0.50, 0.05], 
             [0.30, 0.0, 0.12],
             start2_0]
    # pdb.set_trace()
    for pt in range(len(Traj1)):
        ...
        MP2= DMPs_cartesian_(K=K, alpha_s=alpha_s, tol=tol)
        ...
        #Main while loop
        with imageio.get_writer('Controlled_alpha.gif', mode='I', fps=10) as writer:
            while total_time < 0.8:
                ...
                while not (flag_conv and flag_conv2):
                    ...
                    if (not flag_conv) and close_flag:
                        ...
                        if flag_conv and not flag_conv2:
                            
                            # position_ee = np.array(start1_0)
                            # positions = np.linspace(np.array(start1_0) - np.array(meas1), 10)
                            # position_ee[1] = position_ee[1] + 1*dt
                            position_ee = start1_0
                            rotation_ee = [0,0,0.707,0.707]
                            joint_angle = robot1._calculateIK(position_ee,rotation_ee)
                            arm1_ee = robot1._getEndEffectorPosition()
                            robot1._sendPositionCommand(joint_angle)

                            desired_x[0].append(arm1_ee[0])
                            measured_x[0].append(arm1_ee[0])

                            desired_y[0].append(arm1_ee[1])
                            measured_y[0].append(l[0][1])

                            desired_z[0].append(arm1_ee[2])
                            measured_z[0].append(arm1_ee[2])
                    ...
                    if not flag_conv2:
                        ...
                        # distance = np.linalg.norm(gamma_tra[-1] - gamma2_tra[-1])
                        arm1_ee = robot1._getEndEffectorPosition()
                        distance = np.linalg.norm(np.array(list(arm1_ee)) - gamma2_tra[-1])
                        distance = distance if distance < r_s else np.inf
                        MP2.step(external_force=perturb_dynamic_agent2, distance=distance)
                        ...
                        if flag_conv2 and not flag_conv and pt == len(Traj1)-2:
                            MP2.goal = start2_0
                            flag_conv2 = np.linalg.norm(meas2- MP2.x_goal) < 0.05
        ...
        time.sleep(1.0)
        if pt == 0:
            box_id1 = pb.loadURDF("Robot_Sim/urdf/object/box/box.urdf", (0.3, 0.5, 0.025))
            box_id2 = pb.loadURDF("Robot_Sim/urdf/object/box/box.urdf", (0.3, -0.5, 0.025))
            box_id3 = pb.loadURDF("Robot_Sim/urdf/object/box/box.urdf", (0.45, 0.45, 0.025))
