...
def compute_collision_ellip(robot):
    ...
    Link_gloablpose = getlink_global_pose(positions, orientations)

    return Link_gloablpose
...
if __name__=="__main__":
    ...
    robot2.initialize(base_pos=[0.5, -0.5, 0.0],base_ori=[0,0,0,-1])
    ...
    # obstacle
    center_e=np.array([10.0,1.0,0.25])
    ...
    axes_agent=np.array([0.01,0.01,0.01])
    ...
    while total_time < 1:
        ...
        while not (flag_conv and flag_conv2):
            ...
            if (not flag_conv) and close_flag:
                ...
                compute_collision_ellip(robot1)
