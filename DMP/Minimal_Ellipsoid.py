...
def find_minimum_volume_ellipsoid(points):
    ...
    # # Calculate the maximum distances along x, y, and z for initial semi-axis lengths
    # max_distances = np.max(points - centroid, axis=0)

    # # Use these as the initial guess for the semi-axis lengths
    # initial_guess = np.hstack((centroid, max_distances + 1e-3))  # Add a small value to avoid zero
