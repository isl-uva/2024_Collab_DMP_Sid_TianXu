...
class Obstacle_Static():
    ...
    def gen_external_force_rotate(self, x, R=np.eye(3)):
        '''
        Compute the forcing term generated by the potential U as
          varphi(x) = - nabla U
        '''
        ...
        '''
        R = R_{map} @ R_{ellipsoid_rotate}
        '''


        x= R.T @ (x -self.center) + self.center
...
class Obstacle_Dynamic():
    ...
    def gen_external_force_rotate(self, x, v, R=np.eye(3)):
        '''
        Compute the forcing term
         - nabla_x U(x, v)
        '''
