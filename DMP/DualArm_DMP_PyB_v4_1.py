...
from GlobalPose import getlink_global_pose
...
def compute_collision_ellip(robot):
    positions = []
    orientations = []
    Link_velocities = []
    for l in range(robot.num_joints):
        Link_states = robot._getLinkState(l)[0:2]
        positions.append(Link_states[0])
        orientations.append(Link_states[1])
        velocity = robot._getLinkState(l)[6]
        Link_velocities.append(velocity)
        # print('Positions --->',Link_states)

        # link_state = robot._getLinkState(l)
        # print(f"Link {l} state: ")
        # print(f"  Position: {link_state[0]}")
        # print(f"  Orientation: {link_state[1]}")
        # print(f"  World Link Frame Position: {link_state[4]}")
        # print(f"  World Link Frame Orientation: {link_state[5]}")
        
        # # Velocities
        # print(f"  Linear Velocity: {link_state[6]}")
        # print(f"  Angular Velocity: {link_state[7]}")
        # print('*' * 30)

        # pdb.set_trace()
    
    Link_gloablpose, Link_globalvel = getlink_global_pose(positions, orientations, Link_velocities)
...
if __name__=="__main__":
    ...
    robot2.initialize(base_pos=[0.5, -0.5, 0.0])
    ...
    data_dict_agent1, velocity_dict_agent1 = compute_collision_ellip(robot1)
    data_dict_agent2, velocity_dict_agent2 = compute_collision_ellip(robot2)
