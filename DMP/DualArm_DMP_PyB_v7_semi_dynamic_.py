...
def perturb_dynamic_agent1(x, v):    
    ...
    for i in range(1, num_links+1):
        ...
        out+=Ellipsoid_dict_agent2[key].gen_external_force_rotate(x, v, R1@rotm_agent2[key])
...
def perturb_dynamic_agent2(x, v):
    ...
    for i in range(1, num_links+1):
        ...
        out+=Ellipsoid_dict_agent1[key].gen_external_force_rotate(x, v, R2@rotm_agent1[key])
...
if __name__=="__main__":
    ...
    robot1.initialize(base_pos=[0.0, 0.2, 0.0], base_ori=[0,0,0.707, 0.707])
    robot2.initialize(base_pos=[0.5, 0.2, 0.0], base_ori=[0,0,0.707, 0.707])
    ...
    for i in range(1, num_links+1):
        ...
        Ellipsoid_dict_agent1[f"link{i}"]=sq_dyn(center=center_map, axis=axes_map+axes_safe, coeffs=np.array([1,1,1]), lmbda=dyn_lambda, beta=dyn_beta, eta=dyn_eta)

        center_map=cent_agent2[f"link{i}"]
        center_map=tra_map_forward(center_map, alpha1, T1, R1)
        axes_map=axes_agent1[f"link{i}"]
        axes_map=alpha1 * axes_map
        Ellipsoid_dict_agent2[f"link{i}"]=sq_dyn(center=center_map, axis=axes_map+axes_safe, coeffs=np.array([1,1,1]), lmbda=dyn_lambda, beta=dyn_beta, eta=dyn_eta)
